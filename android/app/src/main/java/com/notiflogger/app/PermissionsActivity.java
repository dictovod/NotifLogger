...
public class PermissionsActivity extends AppCompatActivity {
    ...
    private void setupClickListeners() {
        ...
        phoneButton.setOnClickListener(v -> showPhoneInfo());
    }
    ...
    private void updateStoragePermission() {
        boolean hasPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.R ?
    ...
    private void updatePhoneInfo() {
        phoneIcon.setImageResource(android.R.drawable.ic_dialog_info);
    ...
    private void requestNotificationPermission() {
        if (Utils.isNotificationServiceEnabled(this)) {
            ...
            showPermissionDialog(
                ...
                "Приложению необходим доступ к уведомлениям для их логирования. Это основная функция приложения.\n\nВ настройках найдите \"" + getString(R.string.app_name) + "\" и включите доступ.",
                () -> {
                    Intent intent = new Intent(Settings.ACTION_NOTIFICATION_LISTENER_SETTINGS);
    ...
    private void showPhoneInfo() {
        showPermissionDialog(
            ...
            "Приложение больше не требует разрешения на доступ к состоянию телефона (IMEI). Теперь используется уникальный идентификатор устройства (Device ID), который получается безопасно и не требует специальных разрешений.\n\nЕсли у вас есть вопросы, обратитесь в поддержку.",
            null
        );
    }
